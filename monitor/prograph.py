...
def geraGraphico(_col_pontos, _col_campanha, _parametro,  _idleg, _idclasse):
    ...
    template = """{chart: {width:800, plotBackgroundColor: '#E6E6E6', borderColor: 'gray', borderWidth:1 },
title: { text: ' ' },
xAxis: [{ categories: {{categories|safe}} , crosshair: true, labels: {style: {fontWeight: 'bold'}}, gridLineWidth:0}],
yAxis: [{title:{ text: '{{ str_param }}',style: {fontWeight: 'bold', color: 'black'} }, labels: { style: {fontWeight: 'bold' }, format: '{value:.{{casas}}f}' }, gridLineWidth:4}],
credits: { text: ' ', href: ' '},
...
"""
    ...
    for item in grade:
        ...
        indice_cor = _col_campanha.index(item)
        if sum(dados) != 0:
            registro = {'name': '{0}'.format(obj_campanha),
                        'type':'column',
                        'data':dados,
                        'color': cores[indice_cor],
                        'order': order}
...
def processa(_col_pontos, _col_campanha, _col_parametro,  _idleg, _idclasse):
    ...
    col_param = []
    for item in Param.objects.all().order_by('nome'):
        if item.id in (_col_parametro):
            col_param.append(item.id)

    col_pontos = [ [], [], []]
    for item in  _col_pontos:
        print(item)
        pto = PtoMonit.objects.get(pk=item)
        zona = -1
        sigla = pto.sigla.upper()
        if sigla.endswith('ZF'):
            ...
            zona = 0
        col_pontos[zona].append(item)

    zonas = [u'Superfície', u' 1/2 Zona Fótica', u'Fundo',]
    ...
    for parametro  in col_param:

        """ Não processa parametros Data e Profundidade"""
        if parametro in (798,840):
            continue

        is_primeiro_param = True


        indice_pontos = 0
        for it_pontos in col_pontos:

            if  len(it_pontos) == 0:
                continue


            saida = geraGraphico(it_pontos, _col_campanha, parametro, _idleg, _idclasse)
            if not saida:
                continue
            ...
            if is_primeiro_param:
                obj_param = Param.objects.get(pk=parametro)
                nome = obj_param.nome
                texto = obj_param.texto
                document.add_heading(u'6.1.{0} - {1}'.format(indice, nome), level=3)
                document.add_paragraph('', style='Normal')
                document.add_paragraph(texto, style='Normal')
                document.add_paragraph('', style='Normal')
                is_primeiro_param = False

            document.add_paragraph('', style='Normal')
            label = u'FIGURA 6.1.{0}.{1} - Resultados obtidos para o parâmetro {2} - {3}'
            document.add_paragraph(label.format(indice, indice_pontos+1, nome, zonas[indice_pontos] ), style='f')
            document.add_paragraph('', style='Normal')
